...
import { getMe, getStats } from '../../../src/controllers/me';
...
describe('getStats', () => {
    it('should return the user and their chats and token usages', async () => {
        ...
        expect(res.body.data.totalChats).toBe(0);
        expect(res.body.data.avgTokensPerChat).toBe(0);
        expect(res.body.data.avgMessagesPerChat).toBe(0);
        expect(res.body.data.avgTokensPerMessage).toBe(0);
    });
    it('should return a 404 if the user is not found', async () => {
        const req = await createMockAuthRequest({ id: 'test-user-123' });
        const res = createMockResponse();
